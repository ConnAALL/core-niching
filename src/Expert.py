...
class bot(object):
    ...
    max_speed: int = 3 # Modable, 2 + 0-7, 3 bits (0-2)
    desired_speed: int = 3 # Modable, 2 + 0-7, 3 bits (3-5)
    safety_margin: int = 12 # Modable
    turnspeed: int = 20 # Modable, 5 + 3*0-15, 4 bits (6-9)
    power_level: float = 28.0 # Modable, 5 + 3*0-15, 4 bits (10-13)
    scan_distance: int = 1000 # Modable but probably wont change much, must stay over thresholds
    cpa_check_distance = 500 # Modable, 100 + 100*0-7, 3 bits (14 - 15)
    cpa_threshold = 50 # Modable
    dodge_threshold = 50 # Modable
    thrust_heading_tolerance: int = 10 # Modable
    e_thrust_heading_tolerance: int = 15 # Modable
    wall_threshold: int = 50 # Modable
    ...
    def run_loop(self, ) -> None:
        ...
        if debug:
            print(
                f"desired_heading={self.desired_heading}, "
                f"last_radar_heading={self.last_radar_heading}, "
    ...
    def production_system(self,) -> None:
        ...
        '''
        if debug:
            print(f"Turn = {self.turn}, Shoot = {self.shoot}, Thrust = {self.thrust}")
    ...
    def set_flags(self, ) -> None:
        ...
        if self.check_walls():
            ...
            pass
        elif False and self.check_radar():
    ...
    def check_ships(self, ) -> bool:
        ...
        while next_ship_dist != -1 and next_ship_dist < self.cpa_check_distance:
            ...
            if enemy_cpa_distance < self.cpa_threshold:
    ...
    def check_shots(self, ) -> bool:
        ...
        if lowest_alert < self.dodge_threshold:
            ...
            self.desired_heading = self.get_closer_angle(
                left_option, right_option)
            if self.check_heading_tolerance(self.e_thrust_heading_tolerance) or lowest_alert < self.dodge_threshold:
                self.thrust = True
...
def main():
    global bot_name
    bot_name = "EKKO{}".format(sys.argv[1])
    global debug
    debug = False
    global agent
    agent = bot(bot_name)
    ...
    if len(sys.argv) > 3 and (sys.argv[3].lower()) in debug_answers: # If we want to launch in debug mode
        debug = True

    #ai.start(
    #    agent.run_loop, ["-name", bot_name, "-join", "localhost", "-team", "1"])

    ai.start(
        agent.run_loop, ["-name", bot_name, "-join", "localhost"])
